...
function findArrayName(name, data) {
  ...
  try {
    ...
    var y = check[0];
    val = !lodash.isUndefined(data[y][n]) && !lodash.isNull(data[y][n]) ? data[y][n] : null;
  } catch (e) {
    val = !lodash.isUndefined(data[name]) && !lodash.isNull(data[name]) ? data[name] : null;
  }
...
function fetchErrorDispatch(error, dispatch, prefix) {
  ...
  if (prefix != 'NULL') {
    if (!lodash.isEqual(prefix, 'EXTRANET')) {
      dispatch({
    ...
    Object.keys(data).map(function (key, index) {
      if (lodash.isArray(data[key])) {
        var _loop = function _loop(i) {
          if (lodash.isObject(data[key][i])) {
            Object.keys(data[key][i]).map(function (k2, i2) {
              if (!lodash.isUndefined(data[key][i][k2]) && !lodash.isNull(data[key][i][k2])) {
                d.append(key + '[' + i + '][' + k2 + ']', data[key][i][k2]);
              }
            });
          } else {
            if (!lodash.isUndefined(data[key][i]) && !lodash.isNull(data[key][i])) {
      ...
      } else if (lodash.isObject(data[key])) {
        Object.keys(data[key]).map(function (k, i) {
          if (!lodash.isUndefined(data[key][k]) && !lodash.isNull(data[key][k])) {
            d.append(key + '[' + k + ']', data[key][k]);
          }
        });
      } else {
        if (!lodash.isUndefined(data[key]) && !lodash.isNull(data[key])) {
...
function defaultFilterData(data, columns, name) {
  ...
  for (var i = 0; i < columns.length; i++) {
    ...
    try {
      ...
      if (!lodash.isNull(cari) && !lodash.isUndefined(cari)) {
        search[kolom] = cari;
      }
    } catch (e) {}
    ...
    try {
      ...
      if (!lodash.isNull(urut) && !lodash.isUndefined(urut)) {
        sorted.push({
          id: kolom,
          desc: urut == 'desc'
        });
...
exports.authReducer = auth;
exports.coreReducer = core;
exports.defaultFilterData = defaultFilterData;
exports.fetchErrorDispatch = fetchErrorDispatch;
exports.fetchSuccessDispatch = fetchSuccessDispatch;
exports.findArrayName = findArrayName;
